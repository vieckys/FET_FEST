...
childRouter.put("/:id",function(req,res,next){
    if(req.body){
        connection.update("children",req.body,{id:{operator:'=', value:req.body.id}},function(err,rows){
            if(err){
                next(err);
            }
...
childRouter.post("/byUser/:id",async function(req,res,next){
    if(req.body){
        let data=await new Promise((resolve, reject)=>{
            connection.query("SELECT id,name,DATE_FORMAT(dob, \"%d-%m-%Y\") as dob,gender,comments,height,weight,user_id from children where user_id='"+req.body.id+"'",function(err,rows){
                if(err){
                    reject(err);
                }
...
childRouter.post("/:id",function(req,res,next){
    if(req.body){
        connection.query("SELECT id,name,DATE_FORMAT(dob, \"%d-%m-%Y\") as dob,gender,comments,height,weight,user_id from children where id='"+req.body.id+"' AND user_id='"+req.body.user_id+"'",function(err,rows){
            if(err){
                next(err);
            }
            if(rows!=null && rows.length>0){
